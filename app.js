    ...
    _redraw: function () {
        ...
        for (let i = 0; i < this._latRanges.length; i++) {
            ...
            for (let j = 0; j < this._lngRanges.length; j++) {
                ...
                if (rw > 3 && rh > 3) ctx.strokeRect(x + 0.5, y + 0.5, rw - 1, rh - 1);
            }
        }
    }
});
...
function updateMap() {
    ...
    if (totalCombos <= INTERACTIVE_RENDER_THRESHOLD) {
        ...
        const bounds = L.latLngBounds(areas.map(area => [
            [area.latMin, area.lngMin],
            [area.latMax, area.lngMax]
        ]).flat());
        map.fitBounds(bounds, { padding: [50, 50] });
    } else {
        // Moltes combinacions: canvas d'alta densitat (groc superposat al blau)
